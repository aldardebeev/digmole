...
export class CreateUserNotification {
    ...
    async handle(job: Job, conversation: Conversation<CustomContext>) {
        ...
        await this.bot.api.sendMessage(job.data.chatId, 'Подтвердите, что это Ваш кошелек.\n' +
            '1. Скопируйте фразу для подписи в буфер обмена 👇\n')

        await this.bot.api.sendMessage(job.data.chatId, `${job.data.phrase}`)

        await this.bot.api.sendMessage(job.data.chatId,
